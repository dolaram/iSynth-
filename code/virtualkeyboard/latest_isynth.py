...
for jj in range(0,3):
    for ii in range(0,3):
        ...
        keymap.append([xbegin,xend,ybegin,yend,key]);
##############################################################################
inputkey_xcoord=np.array([220,200])
keypressed=0
for i in range(9):
    cond1 = keymap[i][0] <= inputkey_xcoord[1] < keymap[i][1]
    cond2 = keymap[i][2] <= inputkey_xcoord[0] < keymap[i][3]
    if cond1 and cond2:
...
#
#for i in range(0,3) :
#    if keymap[i][0] <= inputkey_xcoord < keymap[i][1] :
